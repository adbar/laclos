...
for filename in listdir(args.inputdir): #  test/ korpus/test/
    ...
    if filecheck.match(filename):
        ...
        # second version
        filecheck2 = re.compile(r'([0-9]+)_[0-9]+_utf-8$')
        if filecheck2.match(filename):
            # skip if the file has been blacklisted
            if filecheck2.match(filename).group(1) in blacklist:
                continue
        else:
            print ('unknown file name pattern, skipping file...')
            continue
    ...
    clean_lines = list()
    queue = list()
    abbrv_flag, quote_flag, upper_flag, flush = 0, 0, 0, 0
    ...
    try:
        ...
        print ('Empty file ? skipping file...', filename)
        continue
        ...
        if line:
            ...
            # SSA
                ...
                if not re.match(r'Dialogue: ', line):
                    ...
                    # print (line)
# Dialogue: 0,0:00:10.56,0:00:12.52,Default,NTP,
# Dialogue: 0,0:00:48.41,0:00:52.38,Default,,0000,0000,0000,,Aber ich sage unserer Bürgerschaft:
                    line = re.sub(r'Dialogue:.+?Effect,', '', line)
                    #line = re.sub(r'Dialogue:.+?NTP,', '', line)
                    #line = re.sub(r'Dialogue:.+?Default,', '', line)
                    line = line.replace('\N', ' ')
                    line = line.replace('\\n', ' ')
                    line = re.sub(r'\{.+?\}', '', line)
